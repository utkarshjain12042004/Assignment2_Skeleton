...
public class HeapSort {
    ...
    private static void heapify(int[] _tree, int _rootindex, int _n) {
        while (2 * _rootindex + 2 <= _n) {
            ...
            if (crIndex < _n && _tree[clIndex] > _tree[crIndex]) { smallest = crIndex; }
            if (_tree[_rootindex] <= _tree[smallest]) { break; }
    ...
    private static void heapify(List<CosSimilarityPair> _tree, int _rootindex, int _n) {
        ...
        while (2 * _rootindex + 2 <= _n) {
            ...
            int clIndex = 2 * _rootindex + 1;  //Index of the left child
            int smallest = clIndex;
            if (crIndex < _n && _tree.get(clIndex).getCosineSimilarity() > _tree.get(crIndex).getCosineSimilarity()){ smallest = crIndex; }
            if (_tree.get(_rootindex).getCosineSimilarity() <= _tree.get(smallest).getCosineSimilarity()) { break; }
            //Swap the root with the largest node.
